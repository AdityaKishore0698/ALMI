...
| G11 | Resuming training isn't exact. (a) `OnPolicyRunner` only advances `current_learning_iteration` after `learn()` returns, so every periodic checkpoint stores `iter: 0` and a resumed run counts from 0 again. (b) Environment curriculum state (arm/motion curriculum weights, terrain levels) isn't saved in the checkpoint, so it restarts from its initial values. | deviation | Record the offset in RUN_LOG (resumed model_N is overall iteration offset+N). Only the policy and optimizer carry over. Fix (a) in `rsl_rl` if resumes become frequent. |
